...
sub _can_delete_storage {
   ...
   if ($s_name ne "") { # if only a name is passed we need to s_query the storage id
      $s_query = "SELECT id FROM STORAGE WHERE name='$s_name';";
      $o_results = get_db_handler()->prepare($s_query);
      $o_results->execute();
      @a_row = $o_results->fetchrow_array();
      if (scalar @a_row == 0) {return 0;} # if no results, storage name does not exists, return false
      $s_id = @a_row[0];
      ...
      $o_results->execute();
      @a_row = $o_results->fetchrow_array();
      if (scalar @a_row == 0) {return 1;} # if no results, then the storage id is not present as a fk_storage and said storage can be deleted
...
sub create_table {
   ...
   my $s_query = "CREATE TABLE IF NOT EXISTS $s_tablename ();";
...
sub delete_table {
   ...
   my $s_query = "DROP TABLE IF EXISTS $s_tablename;";
   get_db_handler()->prepare($s_query)->execute() or die $DBI::errstr;
...
sub add_column_to_table {
   ...
   my $s_query = "ALTER TABLE $s_tablename
...
sub delete_column_from_table {
   ...
   my $s_query = "ALTER TABLE $s_tablename DROP COLUMN $s_col_name;";
   get_db_handler()->prepare($s_query)->execute() or die $DBI::errstr;
...
sub get_table {
   ...
   my $s_query = "SELECT * FROM $s_tablename;";
   my $o_results = get_db_handler()->prepare($s_query);
   ...
   while (my @a_row = $o_results->fetchrow_array()) {
      for (my $i = 0; $i < scalar @a_row; $i++) {
         my $s_pad = " " x (length($col_names->[$i]) - length($a_row[$i])); # calculating empty spaces for padding the table
         print " " .  $a_row[$i] . $s_pad . " |";
      }
      print "\n";
   }
...
sub add_row_to_table {
   ...
   if ($s_tablename eq "vm") {
      ...
      $s_query = "INSERT INTO $s_tablename (name, capacity, created_on, last_modified)
...
sub get_rows_from_table {
   ...
   my $s_query = "SELECT * FROM $s_tablename WHERE $s_condition;";
   my $o_results = get_db_handler()->prepare($s_query);
   my $_results = $o_results->execute() or die $DBI::errstr;
...
sub get_col_from_table {
   ...
   my $s_query = "SELECT $s_col FROM $s_tablename;";
   my $o_results = get_db_handler()->prepare($s_query);
   my $_results = $o_results->execute() or die $DBI::errstr;
...
sub add_storage_to_vm {
   ...
   if ($s_storage_id eq "") {
      ...
      $o_results->execute();
      @a_row = $o_results->fetchrow_array();
      if (scalar @a_row == 0) {$s_storage_id = 'null';} # if no results, storage name does not exists, set storage_name to null
      $s_storage_id = @a_row[0];
   }

   # setting the storage id as fk_storage in table vm
   if ($s_vm_name ne "") {
      $s_query = "UPDATE vm SET fk_storage=$s_storage_id where name='$s_vm_name';";
      $o_results = get_db_handler()->prepare($s_query);
      $o_results->execute();
      print "added storage_id $s_storage_id as foreign key to vm '$s_vm_name'\n";
   } elsif ($s_vm_id ne "") {
      $s_query = "UPDATE vm SET fk_storage=$s_storage_id where id='$s_vm_id';";
      $o_results = get_db_handler()->prepare($s_query);
      $o_results->execute();
...
sub remove_storage_from_vm {
   ...
   if ($s_name ne "") {
      $s_query = "UPDATE vm SET fk_storage=null where name='$s_name';";
      $o_results = get_db_handler()->prepare($s_query);
      $o_results->execute();
      print "removed storage from vm '$s_name'\n";
   } elsif ($s_id ne "") {
      $s_query = "UPDATE vm SET fk_storage=nullwhere id='$s_id';";
      $o_results = get_db_handler()->prepare($s_query);
      $o_results->execute();
